...
export default function StudentProfilePage() {
  ...
  const [editForm, setEditForm] = useState({ nick_name: "", first_name: "", last_name: "", dob: "", parent_phone: "", parent_line_id: "" });
  ...
  async function handleEditStudent() {
    ...
    const { error } = await supabase.from("students").update({
      ...
      parent_line_id: editForm.parent_line_id || null,
    }).eq("id", student.id);
      ...
      <div className="bg-white rounded-2xl p-5 border" style={{ borderColor: POS.borderPurple, boxShadow: POS.shadowMd }}>
        <div className="flex items-start gap-4">
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={() => { setEditForm({ nick_name: student.nick_name || "", first_name: student.first_name || "", last_name: student.last_name || "", dob: student.dob || "", parent_phone: student.parent_phone || "", parent_line_id: student.parent_line_id || "" }); setEditOpen(true); }}
              aria-label={t("edit")}
            ...
            <div className="flex flex-col items-center gap-2 w-full">
              ...
              <button disabled={!lineUserId.trim() || linkingLine}
                onClick={async () => {
                  ...
                  setLinkingLine(true);
                  const { error: err } = await supabase.from("students").update({ parent_line_id: lineUserId.trim() }).eq("id", id);
      ...
      <Dialog open={editOpen} onClose={() => setEditOpen(false)} className="fixed z-50 inset-0 flex items-center justify-center" style={{ background: "rgba(0,0,0,0.3)" }}>
        <Dialog.Panel className="bg-white rounded-2xl p-6 max-w-md w-full mx-4" style={{ boxShadow: POS.shadowXl }}>
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="text-xs font-semibold" style={{ color: POS.textSecondary }}>LINE ID</label>
              <input type="text" className="w-full rounded-xl border px-3 py-3 mt-1" style={{ borderColor: POS.border }}
                value={editForm.parent_line_id} onChange={e => setEditForm(f => ({ ...f, parent_line_id: e.target.value }))} />
            </div>
          </div>
