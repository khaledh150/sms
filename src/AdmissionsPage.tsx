  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("courses")
        .select("id,name,weekdays,times,capacity").order("name");
...
function usePublicLinkQuery(enabled: boolean) {
  return useQuery({
    ...
    queryFn: async () => {
      // 1. Check for valid link
      const { data: links } = await supabase.from("application_links")
        .select("*").order("created_at", { ascending: false }).limit(1);
      let link = links && links[0] && new Date(links[0].expires_at) > new Date() ? links[0] : null;
      if (!link) {
        // Expire old
        await supabase.from("application_links")
          .update({ expires_at: new Date(Date.now() - 60 * 1000) })
          .neq("expires_at", null);
        // New link
        const { data: newLink } = await supabase.from("application_links")
...
function FloatingMessage({ msg, onClear }) {
  React.useEffect(() => {
    if (!msg) return;
...
export default function AdmissionsPage({ publicMode = false }) {
  const { user } = useAuth();
  ...
  const { data: publicLink, refetch: refetchPublicLink, isFetching: linkLoading } = usePublicLinkQuery(showLink && (role === "admin" || role === "staff"));

  // Query
  const { data: courses, isLoading: loadingC } = useCoursesQuery();

  // Course/day/time logic
  function toggleCourse(cid: string) {
    setSlots((s) => {
      const next = { ...s };
      if (next[cid]) {
        delete next[cid];
        setLimits((l) => { const { [cid]: _, ...rest } = l; return rest; });
      } else {
        next[cid] = {};
      }
      return next;
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    if (!Object.keys(slots).length) return setError("Select at least one course.");
    if (!nick.trim()) return setError("Please enter a nick name.");
    setSaving(true);
    // 1. Upload receipts
    const urls: string[] = [];
    ...
    if (role === "admin") {
      ...
      setFloatMsg(`Student ${nick} added!`); setSubmitted(true);
    } else {
      ...
      if (insErr) { setError(insErr.message); setSaving(false); return; }
      setFloatMsg("Application submitted!"); setSubmitted(true);
    }
    setSaving(false);
  }
    ...
    return (
      <div className="min-h-[60vh] flex flex-col justify-center items-center bg-[#f6f6f6]">
        <div className="bg-gradient-to-br from-blue-50 via-green-50 to-yellow-50 shadow-xl rounded-3xl px-8 py-12 max-w-md w-full flex flex-col items-center">
          <div className="text-3xl font-extrabold mb-3 text-[#6654b3]">Thank you!</div>
          <div className="text-lg text-gray-700 mb-4 text-center">
            Your application has been received.<br />
            Our staff will contact you via LINE or phone.<br />
  ...
  return (
    <div className="flex flex-col gap-8 py-10 px-2 sm:px-6 max-w-3xl mx-auto" style={{ background: "#f6f6f6" }}>
      <header className="mb-4">
        <h1 className="text-3xl font-extrabold text-[#6654b3]">Wonder Kids Admissions</h1>
        <p className="text-lg text-[#6654b3] mt-2">
          Fill out the application form below. {role === "admin" ? "Students will be enrolled instantly." : "Our staff will contact you to confirm your admission."}
        </p>
      </header>
      <form onSubmit={handleSubmit} className="space-y-8">
        {/* Student Info */}
        <div className="bg-white border border-[#ece6fc] p-6 rounded-2xl shadow transition-all">
          <h2 className="text-xl font-bold text-[#6654b3] mb-2">Student Info</h2>
          <div className="grid md:grid-cols-3 gap-4">
            <label className="flex flex-col gap-1">
              <span className="font-semibold text-[#6654b3]">Nick Name</span>
            ...
            <label className="flex flex-col gap-1">
              <span className="font-semibold text-[#6654b3]">First Name</span>
              <input required className="border border-[#ece6fc] bg-purple-50 rounded-xl px-4 py-2"
                     value={first}
            ...
            <label className="flex flex-col gap-1">
              <span className="font-semibold text-[#6654b3]">Last Name</span>
              <input required className="border border-[#ece6fc] bg-purple-50 rounded-xl px-4 py-2"
                     value={last}
          ...
          <label className="flex flex-col gap-1 mt-4 w-full md:w-1/2">
            <span className="font-semibold text-[#6654b3]">Date of Birth</span>
        ...
        {/* Guardian */}
        <div className="bg-white border border-[#ece6fc] p-6 rounded-2xl shadow transition-all">
          <h2 className="text-xl font-bold text-[#6654b3] mb-2">Parent / Guardian</h2>
          <div className="grid md:grid-cols-2 gap-4">
            <label className="flex flex-col gap-1">
              <span className="font-semibold text-[#6654b3]">Line App ID</span>
              <input required className="border border-[#ece6fc] bg-purple-50 rounded-xl px-4 py-2"
                     value={mail} onChange={e => setMail(e.target.value)} />
            </label>
            <label className="flex flex-col gap-1">
              <span className="font-semibold text-[#6654b3]">Phone</span>
              <input required className="border border-[#ece6fc] bg-purple-50 rounded-xl px-4 py-2"
        ...
        {/* Courses */}
        <div className="bg-white border border-[#ece6fc] p-6 rounded-2xl shadow transition-all">
          <h2 className="text-xl font-bold text-[#6654b3] mb-2">Courses</h2>
          {loadingC ? <div className="text-gray-400">Loading courses…</div> : (courses?.length === 0 ?
            ...
            : courses.map(c => (
              <div key={c.id} className={`mb-2 border ${slots[c.id] ? "border-[#6654b3] bg-purple-50" : "border-gray-200 bg-white"} rounded-xl p-3 transition-all`}>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-xs text-gray-400">({c.capacity} seats)</span>
                </label>
                {slots[c.id] && (
                  <div className="ml-8 mt-2 mb-2">
                        ...
                        {!!slots[c.id]?.[d] && (
                          <>
                            ...
                            >
                              ...
                              {(c.times[d] || []).filter(t => !slots[c.id][d]?.includes(t)).map((t) => (
                    ...
                    <div className="mt-2">
                      <label className="flex items-center gap-2">
                        <span className="font-semibold text-[#6654b3]">Hours Purchased:</span>
                        <input
                          type="number"
                          min={1}
        ...
        {/* Payment */}
        <div className="bg-white border border-[#ece6fc] p-6 rounded-2xl shadow transition-all">
          <h2 className="text-xl font-bold text-[#6654b3] mb-2">Payment Receipt</h2>
        ...
        <div className="flex justify-end">
          ...
          >
            {role === "admin" ? (saving ? "Adding…" : "Add Student")
        ...
        {(!publicMode && (role === "admin" || role === "staff")) && (
          <div className="mt-12 flex flex-col items-center gap-2">
            ...
            >
              {publicLink ? "Refresh Public Admissions Link" : "Show Public Admissions Link"}
            </button>
            {showLink && publicLink &&
              <div className="flex items-center gap-2 mt-2 bg-[#ece6fc] px-4 py-2 rounded-xl w-full max-w-lg">
                ...
                <button onClick={() => navigator.clipboard.writeText(publicLink)}
                  className="text-[#6654b3] hover:underline font-bold px-2">Copy</button>
              </div>
            }
          </div>
        )}
